...
namespace server.Application.Services
{
    public class ProfessionService : IProfessionService
    {
        ...
        public async Task<ProfessionDto> GetProfessionAsync(int id)
        {
            return new ProfessionDto(await _professionRepository.GetProfession(id));
        }

        public async Task<List<CourseDto>> GetProfessionCoursesAsync(int id)
        {
            return _professionRepository.GetProfessionCourses(id).Result.Select(course => new CourseDto(course)).ToList();
        }

        public async Task<List<SkillDto>> GetProfessionSkillsAsync(int id)
        {
